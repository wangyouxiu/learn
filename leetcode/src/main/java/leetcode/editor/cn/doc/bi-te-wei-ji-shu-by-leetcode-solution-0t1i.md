...
为了判断一个正整数是不是 *2* 的整数次幂，可以利用方法一中提到的按位与运算的性质。如果正整数 *y* 是 *2* 的整数次幂，则 *y* 的二进制表示中只有最高位是 *1*，其余都是 *0*，因此 ![y~\&~(y-1)=0 ](./p__y~&~_y-1_=0_.png) 。由此可见，正整数 *y* 是 *2* 的整数次幂，当且仅当 ![y~\&~(y-1)=0 ](./p__y~&~_y-1_=0_.png) 。
